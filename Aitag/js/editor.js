...
this.stylesheet = "../inc/ie4.css"; // full URL to stylesheet
...
function editor_generate(objname,userConfig) {
 ...
 if (!config.debug) { document.all[objname].style.display = "none"; }
  ...
  //document.all["mccontent1"].value = editor
...
function editor_action(button_id) {
  ...
  // help popup
  if (cmdID == 'showhelp') {
      window.open(_editor_url + "/Scripts/editor_help.html", 'EditorHelp');
    return;
  }

  // popup editor
  if (cmdID == 'popupeditor') {
      window.open(_editor_url + "/Scripts/fullscreen.html?" + objname,
                'FullScreen',
  ...
  // fgColor and bgColor
    ...
    var newcolor = showModalDialog(_editor_url + "/Scripts/select_color.html", oldcolor, "resizable: no; help: no; status: no; scroll: no;");
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
        ...
        showModalDialog(_editor_url + "../insert_image/index", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
       // showModalDialog(_editor_url + "/Scripts/insert_image.html", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }

    // insert table
    else if (cmdID.toLowerCase() == 'inserttable'){
        showModalDialog(_editor_url + "/Scripts/insert_table.html?" + objname,
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
    }
...
function editor_about(objname) {
    showModalDialog(_editor_url + "/Scripts/about.html", window, "resizable: yes; help: no; status: no; scroll: no; ");
}

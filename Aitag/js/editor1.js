...
function editor_action(button_id) {
  ...
  // help popup
  if (cmdID == 'showhelp') {
      window.open(_editor_url + "html/editor_help.html", 'EditorHelp');
    return;
  }

  // popup editor
  if (cmdID == 'popupeditor') {
      window.open(_editor_url + "html/fullscreen.html?" + objname,
                'FullScreen',
  ...
  // Custom2
    ...
    var myText = showModalDialog(_editor_url + "html/custom2.html",
  ...
  // fgColor and bgColor
    ...
    var newcolor = showModalDialog(_editor_url + "html/select_color.html", oldcolor, "resizable: no; help: no; status: no; scroll: no;");
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
        ...
        showModalDialog(_editor_url + "html/insert_image.html", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }

    // insert table
    else if (cmdID.toLowerCase() == 'inserttable'){
        showModalDialog(_editor_url + "html/insert_table.html?" + objname,
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
    }
...
function editor_about(objname) {
  showModalDialog(_editor_url + "html/about.html", window, "resizable: yes; help: no; status: no; scroll: no; ");
}

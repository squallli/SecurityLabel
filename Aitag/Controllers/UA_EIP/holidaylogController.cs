...
namespace Aitag.Controllers
{
    ...
    public class holidaylogController : BaseController
    {
                ...
                else
                {
                    ...
                    using (SqlConnection conn = dbobj.get_conn("Aitag_DBContext"))
                    {
                        ...
                        else
                        {
                            ...
                            //====
                            //20170103 日期區間(26~25) 之前的不予新增
                            //string DateEx = "", NowACDmin = "", NowACDmax = "", hlogsdate = "";
                            //dbobj.get_ACD(DateTime.Now, "min", out NowACDmin, out DateEx);
                            //dbobj.get_ACD(DateTime.Now, "max", out NowACDmax, out DateEx);
                            //dbobj.get_ACD(Request["hlogsdate"], "max", out hlogsdate, out DateEx);
                            //if (DateTime.Parse(NowACDmax)  > DateTime.Parse(hlogsdate))
                            //{
                            //    ViewBag.ErrMsg = @"<script>alert(""不予新增" + NowACDmin + @"之前的資料!!"");</script>";
                            //    return View(col);
                            //}
                            //請假當月 寬限期下月份10號
                            DateTime hlogsdate = DateTime.Parse(Request["hlogsdate"]);
                            hlogsdate = hlogsdate.AddMonths(1);
                            hlogsdate = DateTime.Parse(hlogsdate.Year + "/" + hlogsdate.Month + "/10");

                            if (DateTime.Now > hlogsdate)
                            {
                                ViewBag.ErrMsg = @"<script>alert(""不予新增" + hlogsdate.Year + "/" + hlogsdate.Month + "/1" + @"之前的資料!!"");</script>";
                                return View(col);
                            }
                            //====
        ...
        private void get_ACD(dynamic d, string p, out string outd, out string DateEx)
        {
        ...
        public ActionResult holidaylogtracedel(string id, int? page)
        {
            ...
            else
            {
                NDcommon dbobj = new NDcommon();
                //int tmpcount = 0;
                string  xhlogstime = "", xhlogetime = "", sysnote = "", mailtitle="";

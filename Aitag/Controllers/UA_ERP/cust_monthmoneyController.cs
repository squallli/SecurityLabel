...
{
    [DoAuthorizeFilter]
    public class cust_monthmoneyController : BaseController
    {
        ...
        public ActionResult list(int? page, string orderdata, string orderdata1)
        {
            ...
            string qbseason = "", qslyear = "", qslmonth = "", qvendcomid = "";
            ...
            {
                string sqlstr = "";
                sqlstr = "select * from vend_monthmoney where comid = '" + Session["comid"].ToString() + "' and vtype='1'  and";

                if (qvendcomid != "")
                    sqlstr += " custcomid in (select comid from allcompany where comid like '%" + qvendcomid + "%' or comtitle like '%" + qvendcomid + "%')   and";
        ...
        public ActionResult adddo(string sysflag, int? page, string orderdata, string orderdata1)
        {
            ...
            string qbseason = "", qslyear = "", qslmonth = "", qvendcomid = "", qsdate = "", qedate = "";
            ...
            using (AitagBill_DBContext con = new AitagBill_DBContext())
            {
                ...
                tmpssql = "select vendno, isnull(sum(psummoney),0) as psummoney FROM cust_contractinvclose INNER JOIN cust_contractinvclose_det ON cust_contractinvclose.vcinvid =cust_contractinvclose_det.vcinvid where vstatus <> 'D' ";

                switch (qbseason)
                {
                    case "01"://月獎                       
                        tmpssql += " and slyear =" + qslyear + " and slmonth = " + qslmonth + "  group by vendno ";
                        break;
                    case "02"://季獎 3 / 6 / 9/ 12     
                        ...
                        tmpssql += " and (slyear * 365 + slmonth * 30) >= " + strym.ToString() + " and (slyear * 365 + slmonth * 30)  <= " + etrym.ToString() + " group by vendno ";
                        break;
                    case "03"://半年獎 06 / 12
                        strym = 365 * int.Parse(qslyear) + 30 * int.Parse(qslmonth);
                        //etrym = qslyear.ToString() + qslmonth.ToString().PadLeft(2, '0');
                        ...
                        break;
                    case "04"://年獎                        
                        tmpssql += " and year(vadate) =" + qslyear + " group by vendno ";
                        break;
                }
                 ...
                 if (dr.HasRows)
                 {
                     while (dr.Read())
                     {
                         ...
                         //判斷是否有交易條件,無交易條件,就用xxxxxxxx這個當規則處理 20170107 Mark
                         string vendno = "";
                         tmpssql = "select * from allcompany_rate where allcomid='" + dr["vendno"].ToString() + "'";
                         ...
                         {
                             vendno = dr["vendno"].ToString();
                         }
                         dr1.Close();
                         dr1.Dispose();

                         //單筆金額-規則
                         tmpssql = "select * from allcompany_rate where bseason='" + qbseason + "' and allcomid='" + vendno + "' and btype = '0'";
                         ...
                         //單筆比例-規則
                         tmpssql = "select * from allcompany_rate where bseason='" + qbseason + "' and allcomid='" + vendno + "' and btype = '1'";
                         dr1 = dbobj.dbselect(erpconn1, tmpssql);
                         if (dr1.Read())
                         {
                         ...
                         //落點金額比例-規則
                         tmpssql = "select * from allcompany_rate where bseason='" + qbseason + "' and allcomid='" + vendno + "' and btype = '2' and (bsmoney <=" + tmppsummoney + " and  bemoney >=" + tmppsummoney + ")";
                         dr1 = dbobj.dbselect(erpconn1, tmpssql);
                         if (dr1.Read())
                         {
                             iftax = dr1["brate"].ToString();
                         ...
                         //累積金額比例-規則
                         tmpssql = "select * from allcompany_rate where bseason='" + qbseason + "' and allcomid='" + vendno + "' and btype = '3' and (bsmoney <=" + tmppsummoney + " and  bemoney >=" + tmppsummoney + ")";
                         dr1 = dbobj.dbselect(erpconn1, tmpssql);
                         if (dr1.Read())
                         {
                             iftax = dr1["brate"].ToString();
                         ...
                         addobj.custcomid = dr["vendno"].ToString();
                         addobj.vtype = "1";//1:收入 2:支出
            ...
            tmpform += "<form name='qfr1' action='/cust_monthmoney/list' method='post'>";
        ...
        public ActionResult Delete(string id, int? page, string orderdata, string orderdata1)
        {
            ...
            else
            {
                using (AitagBill_DBContext con = new AitagBill_DBContext())
                {
                    ...
                    {
                        string vcno = dbobj.get_dbvalue(conn1, "select ('年/月：' + convert(char,slyear)+ '/'+convert(char,slmonth)+ ',客戶：' + custcomid) as st1 from vend_monthmoney where vsid='" + condtionArr[i].ToString() + "'");

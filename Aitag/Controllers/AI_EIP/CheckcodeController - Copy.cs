...
namespace Aitag.Controllers
{
    ...
    public class CheckcodeController : BaseController
    {
        ...
        public ActionResult List(int? page, string orderdata, string orderdata1)
        {
            ...
            ViewBag.orderdata = orderdata;
            ViewBag.orderdata1 = orderdata1;
            string qchkclasstitle = "", qchkitem = "";
            if (!string.IsNullOrWhiteSpace(Request["qchkclasstitle"]))
            {
                qchkclasstitle = Request["qchkclasstitle"].Trim();
                ViewBag.qchkclasstitle = qchkclasstitle;
            }
            if (!string.IsNullOrWhiteSpace(Request["qchkitem"]))
            {
            ...
            using (Aitag_DBContext con = new Aitag_DBContext())
            {
                ...
                if (qchkclasstitle != "")
                    sqlstr += " chkclasstitle like '%" + qchkclasstitle + "%'  and";
                if (qchkitem != "")

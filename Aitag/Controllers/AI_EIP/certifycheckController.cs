...
{
     [DoAuthorizeFilter]
    public class certifycheckController : BaseController
    {
        ...
        //    certifycheck col = new certifycheck();
        //    return View(col);
        //}

        //[HttpPost]
        public ActionResult add(certifycheck col, string sysflag, int? page, string orderdata, string orderdata1)
        {
            ModelState.Clear();
            ...
            if (sysflag != "A")
            {             
                certifycheck newcol = new certifycheck();
                return View(newcol);
            }
            else
            {    
                ...
                else
                {
                    ...
                    string sqlstr = "select * from certifycheck where 1<>1";
                    ...
                    */
                    ...
                    using (Aitag_DBContext con = new Aitag_DBContext())
                    {
                        ...
                        string sysnote = "認證申請編號：" + col.cerno ;
                    ...
                    tmpform += "<form name='qfr1' action='/certifycheck/List' method='post'>";
        ...
        [ValidateInput(false)]
        public ActionResult Edit(certifycheck chks, string sysflag, int? page, string orderdata, string orderdata1)
        {
            ModelState.Clear();
            page = ((!page.HasValue || page < 1) ? 1 : page);
            ViewBag.page = page;
            ...
            if (sysflag != "E")
            {
                using (Aitag_DBContext con = new Aitag_DBContext())
                {
                    var data = con.certifycheck.Where(r => r.cerno == chks.cerno).FirstOrDefault();
                    certifycheck ecertifychecks = con.certifycheck.Find(chks.cerno);
                    if (ecertifychecks == null)
                    {
                        return HttpNotFound();
                    }
                    return View(ecertifychecks);
                }
                ...
                else
                {
                    ...
                    using (Aitag_DBContext con = new Aitag_DBContext())
                    {
                        ...
                        //================================================= //                     
                         ...
                         string sysrealsid = Request["sysrealsid"].ToString();
                         string syssubname = dbobj.get_sysmenuname(sysconn, sysrealsid, "2");
                         string sysnote = "認證申請編號：" + chks.cerno;
                         dbobj.systemlog(sysconn, syssubname, sysnote, Session["tempid"].ToString(), Session["sldate"].ToString(), Session["sfip"].ToString(), sysflag);
                         sysconn.Close();
                         sysconn.Dispose();
                        //=================================================
        ...
        public ActionResult List(int? page, string orderdata, string orderdata1)
        {
            ...
            IPagedList<certifycheck> result;
            using (Aitag_DBContext con = new Aitag_DBContext())
            {
                string sqlstr = "select * from certifycheck where";
                if (qcerno != "")
                ...
                var query = con.certifycheck.SqlQuery(sqlstr).AsQueryable();

                result = query.ToPagedList<certifycheck>(page.Value - 1, (int)Session["pagesize"]);

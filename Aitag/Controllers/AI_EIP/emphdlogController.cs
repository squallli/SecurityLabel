...
namespace Aitag.Controllers
{
    ...
    public class emphdlogController : BaseController
    {
        ...
        public ActionResult empholiday(int? page, string orderdata, string orderdata1)
        {
            ...
            using (Aitag_DBContext con = new Aitag_DBContext())
            {
                string sql = "select CAST(CAST(NEWID() AS binary(3)) AS int) as hid, slyear,empid, empname , 'n' as hchange , NULL as factsday, dptid, '' as hdayid, NULL as effectiveday, 0.0 as allhour, 0.0 as usehour, '' as comid, '' as bmodid, NULL as bmoddate, 0.0 as careeryear  from emphdlog where comid='" + Session["comid"].ToString() + "'  and";
        ...
        public ActionResult empholidaybatch_newAL(emphdlog col, int? page, string orderdata, string orderdata1)
        {
            page = ((!page.HasValue || page < 1) ? 1 : page);
            ViewBag.page = page;
            ...
            //抓年月日
            string qslyearStr = Request["qslyear"].Trim();

            DateTime dtNow = DateTime.Now;
            dtNow = Convert.ToDateTime(qslyearStr + "/" + dtNow.Month + "/" + dtNow.Day);
            dtNow = dtNow.AddMonths(-1);/*前一月*/
            int qslyear = dtNow.Year;
            int qMonth = dtNow.Month;
            int qDay = DateTime.DaysInMonth(qslyear, qMonth);/*年月最後一天*/


            //TimeSpan careerjobdate = new TimeSpan();
            //decimal careeryear = 0;

            //DateTime jobdate = DateTime.Parse("2016/2/6");
            //careerjobdate = DateTime.Now.Subtract(jobdate);
            //careeryear = careerjobdate.Days / 365;
            //careerjobdate = careerjobdate;

            //先繞emphdlog 當年度的 empid。排除掉不新增。
            string no_empid = "";
            using (SqlConnection conn = dbobj.get_conn("Aitag_DBContext"))
            {
                sql = "select empid from emphdlog where slyear = " + qslyear;
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    SqlDataReader dr = cmd.ExecuteReader();
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            no_empid += "'" + dbobj.get_dbnull2(dr["empid"]) + "',";
                        }
                        no_empid = no_empid.Substring(0, no_empid.Length - 1);
                    }
                    dr.Close();
                }
            }
            using (SqlConnection conn = dbobj.get_conn("Aitag_DBContext"))
            {
                sql = "select empid , empname , empworkdepid , jobdate, yhid from employee " +
                    " where empstatus <> '4' and empworkcomp='" + Session["comid"] + "'";

                if (no_empid != "")
                {
                    sql += " and empid not in (" + no_empid + ")";
                }

                sql += " and not (jobdate is null)  order by empid";

                using (SqlCommand sqlcmd = new SqlCommand(sql, conn))
                {
                    SqlDataReader dr = sqlcmd.ExecuteReader();

                    DateTime effectiveday = new DateTime();
                    TimeSpan careerjobdate = new TimeSpan();
                    decimal careeryear = 0;
                    while (dr.Read())
                    {
                        DateTime jobdate = DateTime.Parse(dbobj.get_dbDate(dr["jobdate"], "yyyy/MM/dd"));
                        careerjobdate = DateTime.Parse(qslyear + "/" + qMonth + "/" + qDay).Subtract(jobdate);
                        careeryear = careerjobdate.Days / 365;

                        if (careeryear > decimal.Parse("0.5") && careeryear < 1)
                        {
                            careeryear = decimal.Parse("0.5");
                            effectiveday = jobdate.AddYears(1).AddDays(-1);
                        }
                        else if (careeryear >= 1)
                        {
                            careeryear = Math.Truncate(careeryear);
                            effectiveday = jobdate.AddYears(Convert.ToInt16(careeryear) + 1).AddDays(-1);
                        }

                        //年假產生
                        int yhhours1 = 0;
                        if (careeryear > decimal.Parse("0.5"))
                        {
                            using (SqlConnection conn1 = dbobj.get_conn("Aitag_DBContext"))
                            {
                                sql = "select * from yearholidaydet where yhid='" + dr["yhid"] + "' and comid='" + Session["comid"] + "' and yhsyear <= " + careeryear + " and yheyear > " + careeryear;
                                using (SqlCommand cmd1 = new SqlCommand(sql, conn1))
                                {
                                    ...
                                    {
                                        dr1.Read();
                                        yhhours1 = Convert.ToInt32(dr1["yhours"]);
                            ...
                            col.empname = dr["empname"].ToString();
                            col.dptid = dr["empworkdepid"].ToString();
                            col.effectiveday = effectiveday;
                            col.hdayid = "A01";
                            col.allhour = yhhours1;
                            col.usehour = 0;
                            //col.hchange = "n";

                            col.careeryear = careeryear;
                            ...
                            {
                                sql = "SELECT yearhddet.hdayid,yearhddet.allhour FROM yearholiday INNER JOIN yearhddet ON yearholiday.yhid = yearhddet.yhid AND yearholiday.comid = yearhddet.comid" 
                                    + " where yearholiday.yhid='" + dr["yhid"] 
                                    + "' and yearholiday.comid='" + Session["comid"] 
                                    + "' order by yearhddet.hdayid";
                                ...
                                using (SqlCommand cmd1 = new SqlCommand(sql, conn1))
                                {
                                    ...
                                    while (dr1.Read())
                                    {
                                        ...
                                        col.hdayid = dr1["hdayid"].ToString();
                                        col.allhour = Convert.ToDecimal(dr1["allhour"]);
            ...
            string sysrealsid = Request["sysrealsid"].ToString();
            string syssubname = dbobj.get_sysmenuname(sysconn, sysrealsid, "2");
            string sysnote = "批次新增年度:" + qslyear + "、月份:" + qMonth + "，所有員工的年假資料";
            string sysflag = "A";
            dbobj.systemlog(sysconn, syssubname, sysnote, Session["tempid"].ToString(), Session["sldate"].ToString(), Session["sfip"].ToString(), sysflag);
            sysconn.Close();
            sysconn.Dispose();

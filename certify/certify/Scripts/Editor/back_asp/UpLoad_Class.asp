	...
	Private Function GetErr(ByVal Num)
		Select Case Num
			Case 0
				GetErr = "数据处理完毕!"
			Case 1
				GetErr = "上傳檔案大小超過" & GetSize(vMaxSize) & "限制!可設置MaxSize屬性來改變限制!"
			Case 2
				GetErr = "未设置上传表单enctype属性为multipart/form-data或者未设置method属性为Post,上传无效!"
			Case 3
				GetErr = "含有非法檔名(" & vErrExe & ")文件!只能上傳以下附檔名" & Replace(vExe, "|", ",") & "的文件"
			Case 4
				GetErr = "对不起,程式不允許使用相同name屬性的文件名稱!"
			Case 5
				GetErr = "單個文字大小超出" & GetSize(vSingleSize) & "的上傳限制!"
		End Select
	End Function
